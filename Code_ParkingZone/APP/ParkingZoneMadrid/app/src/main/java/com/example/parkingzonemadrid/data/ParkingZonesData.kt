 ...
 * (distrito, calle) y calculamos el tipo de zona real (Verde, Azul o Mixta)
 * en base a los colores presentes, igual que hace parking-madrid.es.
 */
    ...
    private fun parseCsvAndAggregateByStreet(context: Context): List<StreetZone> {
        ...
        try {
            context.assets.open(CSV_ASSET_NAME).bufferedReader().use { reader ->
                ...
                reader.forEachLine { rawLine ->
                    ...
                    val street = parts[7].trim().ifBlank { "SIN CALLE" }
                    val colorRaw = parts[9].trim().lowercase()
                    val plazas = parts[11].toIntOrNull() ?: 0

                    val key = "$district|$street"
                    val summary = grouped.getOrPut(key) {
                        MutableStreetSummary(district = district, street = street)
                    }
                    summary.countRows++
                    summary.sumX += x
                    summary.sumY += y
        ...
        return grouped.values
            ...
            .map { summary ->
                ...
                val stableKey = "${summary.district}_${summary.street}"
